...
/// symlinks resolved while the directory still exists. Git prints resolved
/// paths, the config keeps whatever the user typed; comparing raw strings
/// would re-import every tracked worktree under its other spelling.
...
impl AppState {
    ...
    pub fn reconcile_worktrees(
        ...
        // The main checkout lists itself; it is the repository row, not a
        // worktree.
        let listed: Vec<(PathBuf, &WorktreeInfo)> = listed
            .iter()
            .map(|info| (resolve(&info.path), info))
        ...
        for worktree in &repo.worktrees {
            ...
            // is not this case.
            if !listed.iter().any(|(path, _)| *path == resolved) && !resolved.exists() {
        ...
        for worktree in &mut repo.worktrees {
            let resolved = resolve(&worktree.path);
            if let Some((_, info)) = listed.iter().find(|(path, _)| *path == resolved) {
...
mod tests {
    ...
    fn reconcile_updates_branches_and_skips_the_main_checkout() {
        ...
        let outcome = state
            .reconcile_worktrees(repo_id, &listed)
        ...
        assert!(
            state.reconcile_worktrees(repo_id, &listed).is_none(),
    ...
    fn reconcile_keeps_unlisted_worktrees_whose_directory_exists() {
        ...
        assert!(state.reconcile_worktrees(repo_id, &listed).is_none());
